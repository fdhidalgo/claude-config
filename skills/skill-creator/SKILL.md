...
### Step 5: Packaging a Skill (Desktop-only)

Skills in `skills/` (shared) don't need packaging—they're immediately available in Claude Code via symlink.

For **Desktop-only skills** in `skills-desktop/`, package them for upload to Claude Desktop:

```bash
~/Documents/personal_scripts/claude-config/sync-scripts/package-skill.sh <skill-name>
```

This creates a ZIP file on your Desktop that can be uploaded via:
Claude Desktop > Settings > Capabilities > Upload skill

**Alternative: Use the built-in packaging script**

For validation and advanced packaging, use the skill-creator's own packaging script:
